 ...
 */
Ext.define('Ext.form.field.DateRange', {
     ...
     * as defined in {@link Ext.data.EventMappings}.</div></li><ul></div>
     */
    setValue: function(v){
        if(!v) {
            return;
        ...
        else if(v[Ext.data.EventMappings.StartDate.name]){ //object
            this.setDT(v[Ext.data.EventMappings.StartDate.name], 'start');
            if(!this.setDT(v[Ext.data.EventMappings.EndDate.name], 'end')){
                this.setDT(v[Ext.data.EventMappings.StartDate.name], 'end');
            }
            this.allDay.setValue(!!v[Ext.data.EventMappings.IsAllDay.name]);
        }
    },
