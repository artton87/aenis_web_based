Ext.define("Aenis.controller.workflow.inheritance.application.ApproveDlg", {
    extend: 'Ext.app.Controller',
    requires: [
        'Aenis.view.workflow.inheritance.application.ApproveDlg'
    ],

    mixins: [
        'Locale.hy_AM.Common',
        'BestSoft.mixin.Localized',
        'Locale.hy_AM.workflow.inheritance.application.ApproveDlg'
    ],
     ...
     * @return {Aenis.view.workflow.inheritance.application.ApproveDlg}
     */
    showDialog: function(params) {
        ...
        this.oDlg.control({
            '[ref=approveAction]': {
                click: {fn: this.onApproveInheritanceApplication, buffer:BestSoft.eventDelay}
            },
            '[action=cancel]': {
                click:this.onclickCloseBtn
            },
            '[ref=printAction]':{
                click: {fn: this.printWarrant, buffer:BestSoft.eventDelay}
            },
            '[ref=payAction]':{
    ...
    afterRenderApproveDialogForm: function(){
        ...
        this.getApproveWarrantForm().transaction_id = this.oDlg.params.model.getId();
        this.getApproveWarrantForm().tr_type_id = this.oDlg.params.model.get('tr_type_id');
    ...
    onclickPayAction: function(){
        ...
        var dutyValue = parseInt(this.getDutyValue().getValue());
    ...
    printWarrant: function(){
        var model = this.oDlg.params.model;
        Ext.Ajax.request({
            url: 'workflow/warrant/print.php',
            method: 'POST',
            params: {
                init: 1,
            ...
            callback: function(options, success, response) {
                if(Aenis.application.handleAjaxResponse(response))
                {
                    ...
                    {
                        window.open(
                            BestSoft.config.server + 'workflow/warrant/print.php?key='+response.key,
                            '_blank',
                            'resizable=yes'
                        )
                    }
    ...
    onFormActionComplete: function(oForm, action) {
        if(action instanceof Ext.form.action.Submit)
        {
            Ext.MessageBox.alert({
                ...
                msg: this.getApproveWarrantForm().messages.warrant_approved,
    ...
    onApproveInheritanceApplication: function(){
        var oForm = this.getApproveWarrantForm().getForm();
        oForm.submit({params:{
            transaction_id: this.getApproveWarrantForm().transaction_id,
            tr_type_id: this.getApproveWarrantForm().tr_type_id,
