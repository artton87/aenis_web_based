 ...
 * @version 	$Id: Reader.php,v 1.1.1.1 2013/09/07 08:57:47 gevorg.darbinyan Exp $
 */
class Csv_Reader extends Csv_Reader_Abstract {
    ...
    protected function loadRow() {
        ...
        //if (!$this->current = str_getcsv($buffer, $this->dialect->delimiter, $this->dialect->quotechar, $this->dialect->escapechar)) {
        ...
        //detect and skip BOM
        if(0 == $this->position && substr($this->current[0], 0, 3) == pack("CCC", 0xef,0xbb,0xbf))
        {
			$this->current[0] = substr($this->current[0], 3);
        }
