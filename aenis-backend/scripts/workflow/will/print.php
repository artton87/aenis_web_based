...
try{
    if(1 == $_POST['init'])
    {
        ...
        $transaction_code_property =  $oWill->getTransactionPropertyValue($data['transaction_id'], 'transaction_code');
        $transaction_code = $transaction_code_property['label'].': '.$transaction_code_property['value'];

        $transaction_password_property =  $oWill->getTransactionPropertyValue($data['transaction_id'], 'transaction_password');
        $transaction_password = $transaction_password_property['label'].': '.$transaction_password_property['value'];

        $transaction_content_property =  $oWill->getTransactionPropertyValue($data['transaction_id'], 'template_content');
...
//output PDF
$pdf = Template_TCPDF::getTCPDFObject('Կտակ');
$pdf->AddPage();
...
$pdf->Output('will_'.$data['transaction_id'].'.pdf', 'I');
