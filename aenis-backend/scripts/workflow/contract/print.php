...
try{
    if(1 == $_POST['init'])
    {
        ...
        //$content = $oContract->getContractPropertyValue($data['transaction_id'], 'template_content');


        $transaction_code_property =  $oContract->getTransactionPropertyValue($data['transaction_id'], 'transaction_code');
        $transaction_code = $transaction_code_property['label'].': '.$transaction_code_property['value'];

        $transaction_password_property =  $oContract->getTransactionPropertyValue($data['transaction_id'], 'transaction_password');
        $transaction_password = $transaction_password_property['label'].': '.$transaction_password_property['value'];

        $transaction_content_property =  $oContract->getTransactionPropertyValue($data['transaction_id'], 'template_content');
...
//output PDF
$pdf = Template_TCPDF::getTCPDFObject('Պայմանագիր');
$pdf->AddPage();
...
$pdf->Output('contract_'.$data['transaction_id'].'.pdf', 'I');
