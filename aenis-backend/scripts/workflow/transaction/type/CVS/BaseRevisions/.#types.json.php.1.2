...
if(!empty($_REQUEST['ui_type']))
	$search['ui_type'] = $_REQUEST['ui_type'];

$cache_id = 'transaction_types_'.md5(serialize(array($search, $detailed, $lang_ids)));
if(false === ($items = App_Cache()->get($cache_id, true, true)))
{
	$current_id = null;
	$currentItemRef = null;
	$oItems = new Transaction_Types();
	...
	$result = $oItems->getItemsTree($search, $detailed, $lang_ids);
	while($row = $oItems->db_fetch_array($result))
	{
		...
		else
		{
			...
			if($id != $current_id)
			{
				...
				if($detailed)
				{
					$item = array_merge($item, App_Array::pick($row,
						'parent_label',
						'form_template',
						'order_in_list',
						'hidden',
	...
	App_Cache()->set($cache_id, $items, null, array(Transaction_Types::CACHE_TAG), true);
}
