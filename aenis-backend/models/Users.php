...
class Users extends App_Db_Table_Abstract
{
   ...
   /* public function tableName()
	...
	public function getUsers(array $search = array(), array $lang_ids = array())
	{
        ...
        if(isset($search['community_id']))
        {
            $joins[] = 'LEFT JOIN bs_staff_user staff_user ON staff_user.user_id = tbl.id
                        LEFT JOIN bs_staff staff ON staff.id = staff_user.staff_id
                        LEFT JOIN bs_departments dep ON dep.id = staff.dep_id
                        LEFT JOIN bs_notarial_offices notarial_offices ON notarial_offices.id = dep.notarial_office_id';

            $where[] = 'staff_user.is_active = 1 AND tbl_content.lang_id = 1
                        AND notarial_offices.community_id = '.$search['community_id'].'';
        }
		...
		$q = "
			SELECT
				...
				tbl_content.lang_id,
				getUserFullName(tbl.id,tbl_content.lang_id) AS username,
				IF((
					SELECT id
