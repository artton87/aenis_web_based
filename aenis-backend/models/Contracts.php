...
/**
 * Methods for contracts management
 * @author BestSoft
 * @package aenis\Workflow
 */
class Contracts extends Transactions
{
	/**
	 ...
	 */
    public function getContracts($search = array(), $start = 0, $limit = 0, $returnCount = false)
    {
		$where = array(
			"tr_types.ui_type = 'contract'"
		);
		$joins = array(
		...
		{
			foreach($contact_ids as $party_type_id => $contacts)
			{
				...
				foreach($contacts as $value)
				{
					...
					WHERE subjects.del_user_id IS NULL
						  AND rel.tr_id = tr.id
						  AND subjects.n_contact_id =".intval($value)."  AND parties.party_type_id = ".intval($party_type_id)."
					LIMIT 1
				";
				}
			}
			$contact_condition = implode(' UNION ',array_unique($contact_conditions));
			$where[] = '('.$contact_condition.') IS NOT NULL ';
		}*/


		$contact_id_conditions = array();
		//Logger::out($n_contact_ids);
		...
		if(!empty($contact_id_condition))
			$contact_id_conditions[] = $contact_id_condition;
		$party_type_id_condition = empty($party_types) ? '' : '(parties.party_type_id IN ('.implode(',', $party_types).'))';
		if(!empty($party_type_id_condition))
			$party_type_id_conditions[] = $party_type_id_condition;*/
		...
		if(!Acl()->allowed('contract.show_all_contracts'))
		{
			$oStaff = new Staff();
			...
			if(Acl()->allowed('contract.show_notary_contracts'))
			{
    ...
    {
        if(empty($id))
            throw new App_Db_Exception_Table('Contract ID is not specified');
     ...
     * @param $file
     */
    public function approveContract($data, $file)
    {
        ...
        $fileData = $file;
        unset($fileData['file_contract']);

        $file = array(
            'file_contract' => end($file)
        );
