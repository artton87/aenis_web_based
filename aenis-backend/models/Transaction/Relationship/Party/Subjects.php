...
class Transaction_Relationship_Party_Subjects extends HistoricalTable
{
	...
	public function getSubjects($search = array())
	{
		...
		$q = "SELECT
				...
				nct.place_of_residence AS n_place_of_residence,
				nct.death_certificate AS n_death_certificate,
				nct.zip AS n_zip,
				...
				jct.organization_type_short_name AS j_organization_type_short_name,
				jct.organization_type_long_name AS organization_type_long_name,
	...
	public function getSubjectContactNames($search = array())
	{
		...
		while($row = $this->db_fetch_array($result))
		{
			...
			if(!empty($row['n_contact_id']))
				$contact_names[] = '<b>'.$row['n_contact_name'].'</b> ծնված՝ '. App_Date::isoDateTime2Str($row['n_date_of_birth'], true).
                    'թ., անձնագիր՝ '.$row['n_passport_number'].',
                     տրված '.$row['n_authority'].' կողմից '.App_Date::isoDateTime2Str($row['n_given_date'], true).', հաշվառված՝ '.$row['n_address'].'';
			if(!empty($row['j_contact_id']))
				$contact_names[] = '<b>'.$row['j_contact_name'].'</b> '.$row['j_organization_type_short_name'].', ՀՎՀՀ՝ '.$row['j_tax_account'].', Պետ. ռեգիստրի գրանցման համար՝ '.$row['j_registration_number'].'';
		}
	...
	public function addSubject($data)
	{
		...
		if($n_contact_id == 0 && $data['contactType'] == 'natural')
		{
            if(isset($data['serviceData']['passport_number']) || isset($data['serviceData']['social_card_number']))
            {
                ...
                $n_contact_id = $oNatural->existsContact($data['serviceData']);
                ...
                {
                    if($oNatural->existContactFullData($data['serviceData'],$data['from_portal']))
                    {
                        $n_contact_id = $oNatural->existContactFullData($data['serviceData'], $data['from_portal']);
                    }
                    else
                    {
		...
		if($j_contact_id == 0 && $data['contactType'] == 'juridical')
		{
                    if(isset($data['serviceData']['registration_number']) || isset($data['serviceData']['tax_account']) || isset($data['serviceData']['organization_name']))
                    {
                        ...
                        {
                            if($oJuridical->existContactFullData($data['serviceData'],$data['from_portal']))
                            {
                                $j_contact_id = $oJuridical->existContactFullData($data['serviceData'], $data['from_portal']);
                            }
                            else
                            {
