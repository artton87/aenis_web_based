...
/**
 * Methods for wills management
 * @author BestSoft
 * @package aenis\Workflow
 */
class Wills extends Transactions
{
    /**
     ...
     */
    public function getWills($search = array(), $start = 0, $limit = 0, $returnCount = false)
    {
        $where = array(
            "tr_types.ui_type = 'will'"
        );
        $joins = array(
        ...
        if (!empty($search['start_date']) && !empty($search['end_date'])){
            $where[] = 'DATE_FORMAT(tr.lu_date,"%d/%m/%Y") BETWEEN "'.$this->db_escape_string($search['start_date']).'" AND "'.$this->db_escape_string($search['end_date']).'"';
        }
        ...
        {
            $search['tr_type_id'] = Transaction_Types::TRANSACTION_TYPE_WILL;
	 ...
	 * @param $file
	 */
	public function approveWill($data, $file)
	{
